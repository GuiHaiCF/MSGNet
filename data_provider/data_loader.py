...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4                           # 输入序列长度
            self.label_len = 24 * 4                             # 解码器初始输入长度
            ...
            self.pred_len = size[2]
        # init
        assert flag in ['train', 'test', 'val']                 # 确保数据集类型合法
        type_map = {'train': 0, 'val': 1, 'test': 2}            # 数据集类型映射
        self.set_type = type_map[flag]

        self.features = features                                # 特征模式（M/S/MS）                                                                     
        self.target = target                                    # 目标列名
        self.scale = scale                                      # 是否标准化数据
        self.timeenc = timeenc                                  # 时间编码方式（0为手动，1为工具函数
        self.freq = freq                                        # 时间频率（如小时、分钟）
    ...
    def __read_data__(self):
        ...
        cols.remove(self.target)
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + [self.target]]        # 调整列顺序（日期+特征+目标）
        # print(cols)
        ...
        border1s = [0, num_train - self.seq_len, len(df_raw) - num_test - self.seq_len]
        border2s = [num_train, num_train + num_vali, len(df_raw)]
        border1 = border1s[self.set_type]                       # 起始索引
        border2 = border2s[self.set_type]                       # 结束索引
        ...
        if self.scale:
            train_data = df_data[border1s[0]:border2s[0]]
            self.scaler.fit(train_data.values)                  # 仅在训练集上拟合标准化器
        ...
        df_stamp = df_raw[['date']][border1:border2]
        df_stamp['date'] = pd.to_datetime(df_stamp.date)
        if self.timeenc == 0:           # 手动编码（月、日、星期、小时）
            df_stamp['month'] = df_stamp.date.apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp.date.apply(lambda row: row.day, 1)
            df_stamp['weekday'] = df_stamp.date.apply(lambda row: row.weekday(), 1)
            df_stamp['hour'] = df_stamp.date.apply(lambda row: row.hour, 1)
        ...
        self.data_y = data[border1:border2]                     # 目标数据（与输入相同，因自回归）
        self.data_stamp = data_stamp                            # 时间特征

    def __getitem__(self, index):
        s_begin = index
        s_end = s_begin + self.seq_len                          # 输入序列区间
        r_begin = s_end - self.label_len                        # 解码器输入起始位置
        r_end = r_begin + self.label_len + self.pred_len        # 解码器输入+预测区间

        seq_x = self.data_x[s_begin:s_end]                      # 输入序列
        seq_y = self.data_y[r_begin:r_end]                      # 目标序列
        seq_x_mark = self.data_stamp[s_begin:s_end]             # 输入时间特征
        seq_y_mark = self.data_stamp[r_begin:r_end]             # 目标时间特征

        return seq_x, seq_y, seq_x_mark, seq_y_mark

    def __len__(self):
        # 返回数据集总长度（考虑序列长度和预测长度）
        return len(self.data_x) - self.seq_len - self.pred_len + 1
