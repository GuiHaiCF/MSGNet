...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        测试流程：
         1. 加载测试数据
         2. 遍历所有测试batch，构造decoder输入，得到模型预测输出
         3. 收集预测结果与真实值，并对部分结果进行可视化保存（保存为PDF文件）
         4. 调用metric函数计算各项指标，包括：
            - MAE（平均绝对误差）
            - MSE（均方误差）
            - RMSE（均方根误差）
            - MAPE（平均绝对百分比误差）
            ...
            - ND（归一化偏差）
            - NRMSE（归一化均方根误差）
         5. 将指标写入result.txt，并保存预测、真实结果和输入数据到磁盘
        """
        test_data, test_loader = self._get_data(flag='test')
        if test:
            print('loading model')
        ...
        print('nd:{:.4f}, nrmse:{:.4f}, mse:{:.4f}, mae:{:.4f}, rse:{:.4f}, mape:{:.4f}, rmse:{:.4f}'.format(nd, nrmse,mse, mae, rse, mape, rmse))
        f = open("result.txt", 'a')
        f.write(setting + "  \n")
        f.write('nd:{}, nrmse:{}, mse:{}, mae:{}, rse:{}, mape:{}, rmse:{}'.format(nd, nrmse,mse, mae, rse, mape, rmse))
        f.write('\n')
        f.write('\n')
        f.close()
